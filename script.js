...
const defaultLessonPlans = [
    { id: 1, title: "Noah's Ark - God Keeps His Promises", group: "Pre-K (Ages 3-5)", date: "2025-11-03", scripture: "Genesis 6-9", keyverse: '"God keeps his promises." - Gen 9:16', objectives: 'Kids learn that God keeps His promises.', activities: '9:00 - Welcome/Free Play\n9:15 - Worship Song\n9:30 - Bible Story\n9:50 - Craft/Game', materials: 'Rainbow craft supplies, animal figures, boat model' },
    { id: 2, title: "Jesus Loves the Little Children", group: "Pre-K (Ages 3-5)", date: "2025-10-27", scripture: "Mark 10:13-16", keyverse: '"Let the children come to me." - Mark 10:14', objectives: 'Kids feel welcomed and loved by Jesus.', activities: '10:00 - Welcome Activity\n10:15 - Worship Song\n10:30 - Story + Puppet Time\n10:50 - Coloring Craft', materials: 'Coloring pages, crayons, puppets' },
    { id: 3, title: "Creation - God Made Everything", group: "Pre-K (Ages 3-5)", date: "2025-10-20", scripture: "Genesis 1-2", keyverse: '"In the beginning, God created..." - Gen 1:1', objectives: 'Kids discover God created everything good.', activities: '9:00 - Sensory bins\n9:20 - Story Cards\n9:40 - Creation Song\n10:00 - Play-Dough Craft', materials: 'Creation cards, sensory bins, play-dough' }
];

let LESSON_PLANS = loadLessonPlans();
...
// Lesson modal context tracking
let lastViewedLessonId = null;
let lastEditPlanId = null;
let lastEditOrigin = null;
...
const LESSON_PLAN_STORAGE_KEY = 'bccLessonPlans';
...
function loadLessonPlans() {
    ...
    try {
        const stored = window.localStorage.getItem(LESSON_PLAN_STORAGE_KEY);
        if (!stored) {
            window.localStorage.setItem(LESSON_PLAN_STORAGE_KEY, JSON.stringify(defaultLessonPlans));
            return [...defaultLessonPlans];
        }
        ...
        console.warn('Unable to load lesson plans from storage.', error);
        return [...defaultLessonPlans];
    }
}

function persistLessonPlans() {
    try {
        if (typeof window !== 'undefined' && window.localStorage) {
            window.localStorage.setItem(LESSON_PLAN_STORAGE_KEY, JSON.stringify(LESSON_PLANS));
        }
    } catch (error) {
        console.warn('Unable to persist lesson plans.', error);
    }
}

function getNextLessonId() {
    return LESSON_PLANS.reduce((maxId, plan) => Math.max(maxId, Number(plan.id) || 0), 0) + 1;
}

function findLessonPlan(id) {
    return LESSON_PLANS.find(plan => String(plan.id) === String(id)) || null;
}

function updateLessonPlan(updatedPlan) {
    LESSON_PLANS = LESSON_PLANS.map(plan => plan.id === updatedPlan.id ? updatedPlan : plan);
    persistLessonPlans();
    renderLessonPlans();
}

function addLessonPlan(newPlan) {
    LESSON_PLANS = [newPlan, ...LESSON_PLANS];
    persistLessonPlans();
    renderLessonPlans();
}

function duplicateLessonPlanData(plan, overrides = {}) {
    const duplicated = {
        ...plan,
        ...overrides,
        id: getNextLessonId(),
    ...
    addLessonPlan(duplicated);
    return duplicated;
}

function collectLessonFormData(form) {
    if (!form?.dataset.planId) return null;
    ...
    if (!title || !group || !date || !activities) {
        return null;
    }

    return {
        id: Number(form.dataset.planId),
        title,
        group,
        date,
        scripture: form['scripture'].value.trim() || 'N/A',
        keyverse: form['keyverse'].value.trim() || 'N/A',
        objectives: form['objectives'].value.trim() || 'N/A',
        activities,
        materials: form['materials'].value.trim() || 'N/A'
    };
}
...
function saveLessonPlan(event) {
    ...
    const newLesson = {
        id: getNextLessonId(),
        title: form['lp-title'].value.trim(),
        group: form['lp-group'].value.trim(),
        date: form['lp-date'].value,
        scripture: form['lp-scripture'].value.trim() || 'N/A',
        keyverse: form['lp-keyverse'].value.trim() || 'N/A',
        objectives: form['lp-objectives'].value.trim() || 'N/A',
        activities: form['lp-activities'].value.trim(),
        materials: form['lp-materials'].value.trim() || 'N/A'
    };

    if (!newLesson.title || !newLesson.group || !newLesson.date || !newLesson.activities) {
        showToast('Please fill in required fields before saving.', true);
        return;
    }

    addLessonPlan(newLesson);
    showToast(`Lesson Plan "${newLesson.title}" saved successfully!`);
...
function openLessonViewModal(plan) {
    const modal = document.getElementById('lesson-plan-view-modal');
    const titleEl = document.getElementById('lesson-plan-view-title');
    const subtitleEl = document.getElementById('lesson-plan-view-subtitle');
    const bodyEl = document.getElementById('lesson-plan-view-body');

    if (!modal || !titleEl || !subtitleEl || !bodyEl) return;

    const formattedDate = plan.date ? new Date(plan.date).toLocaleDateString('en-US', { month: 'long', day: 'numeric', year: 'numeric' }) : 'Date TBD';
    titleEl.textContent = plan.title;
    subtitleEl.textContent = `${plan.group || 'Lesson Plan'} • ${formattedDate}`;

    bodyEl.innerHTML = `
        <div class="detail-block">
            <h4 class="section-title">Overview</h4>
            <p><strong>Scripture</strong><br/>${plan.scripture || 'N/A'}</p>
            <p><strong>Key Verse</strong><br/>${plan.keyverse || 'N/A'}</p>
            <p><strong>Objectives</strong></p>
            <p>${plan.objectives || 'N/A'}</p>
        </div>
        <div class="detail-block">
            <h4 class="section-title">Activities & Flow</h4>
            ${(plan.activities || 'N/A').split('\n').map(step => step.trim()).filter(Boolean).map(step => `<p>${step}</p>`).join('') || '<p>N/A</p>'}
        </div>
        <div class="detail-block">
            <h4 class="section-title">Materials Needed</h4>
            <p>${plan.materials || 'N/A'}</p>
        </div>
    `;

    lastViewedLessonId = plan.id;
    modal.dataset.planId = plan.id;
    toggleModal(modal, true);
}

function openLessonEditModal(plan, options = {}) {
    const modal = document.getElementById('lesson-plan-edit-modal');
    const form = document.getElementById('lesson-plan-edit-form');
    const subtitleEl = document.getElementById('lesson-plan-edit-subtitle');
    const feedbackEl = document.getElementById('lesson-edit-feedback');

    if (!modal || !form || !subtitleEl) return;

    form.reset();

    form.dataset.planId = plan.id;
    subtitleEl.textContent = `${plan.group || 'Lesson Plan'} • ${plan.date ? new Date(plan.date).toLocaleDateString('en-US', { month: 'long', day: 'numeric', year: 'numeric' }) : 'Date TBD'}`;

    form['title'].value = plan.title;
    form['group'].value = plan.group;
    form['date'].value = plan.date;
    form['scripture'].value = plan.scripture === 'N/A' ? '' : plan.scripture || '';
    form['keyverse'].value = plan.keyverse === 'N/A' ? '' : plan.keyverse || '';
    form['objectives'].value = plan.objectives === 'N/A' ? '' : plan.objectives || '';
    form['activities'].value = plan.activities === 'N/A' ? '' : plan.activities || '';
    form['materials'].value = plan.materials === 'N/A' ? '' : plan.materials || '';
    ...
    lastEditPlanId = plan.id;
    lastEditOrigin = options?.origin || null;
...
function viewLessonPlan(id) {
    const plan = findLessonPlan(id);
    if (!plan) {
        showToast('Lesson plan not found.', true);
        return;
    }

    openLessonViewModal(plan);
}
...
function editLessonPlan(id) {
    ...
    openLessonEditModal(plan, { origin: lastViewedLessonId === plan.id ? 'view' : 'dashboard' });
}
...
function duplicateLessonPlan(id) {
    ...
    const duplicated = duplicateLessonPlanData(plan, { date: plan.date });
    showToast(`Lesson Plan "${duplicated.title}" duplicated!`);
    lastViewedLessonId = duplicated.id;
}

function bindLessonPlanEvents() {
    const viewCloseBtn = document.getElementById('lesson-plan-view-close');
    const viewCancelBtn = document.getElementById('lesson-plan-view-cancel');
    const viewEditBtn = document.getElementById('lesson-plan-view-edit');
    const viewDuplicateBtn = document.getElementById('lesson-plan-view-duplicate');

    const editModal = document.getElementById('lesson-plan-edit-modal');
    const editCloseBtn = document.getElementById('lesson-plan-edit-close');
    const editCancelBtn = document.getElementById('lesson-plan-edit-cancel');
    const editDuplicateBtn = document.getElementById('lesson-edit-duplicate');
    const editForm = document.getElementById('lesson-plan-edit-form');

    if (viewCloseBtn) viewCloseBtn.addEventListener('click', closeLessonViewModal);
    if (viewCancelBtn) viewCancelBtn.addEventListener('click', closeLessonViewModal);
    if (viewDuplicateBtn) viewDuplicateBtn.addEventListener('click', duplicateLessonFromView);
    if (viewEditBtn) {
        viewEditBtn.addEventListener('click', () => {
            const modal = document.getElementById('lesson-plan-view-modal');
            if (!modal?.dataset.planId) return;
            const plan = findLessonPlan(modal.dataset.planId);
            closeLessonViewModal();
            if (plan) openLessonEditModal(plan);
        });
    }

    if (editCloseBtn) editCloseBtn.addEventListener('click', closeLessonEditModal);
    if (editCancelBtn) {
        editCancelBtn.addEventListener('click', event => {
            event.preventDefault();
            closeLessonEditModal();
            if (lastEditOrigin === 'view' && lastViewedLessonId) {
                const plan = findLessonPlan(lastViewedLessonId);
                if (plan) openLessonViewModal(plan);
            }
        });
    }

    if (editDuplicateBtn) {
        editDuplicateBtn.addEventListener('click', () => {
            const form = document.getElementById('lesson-plan-edit-form');
            const plan = collectLessonFormData(form);
            if (!plan) {
                const feedbackEl = document.getElementById('lesson-edit-feedback');
                if (feedbackEl) {
                    feedbackEl.textContent = 'Please fill in required fields before duplicating.';
                    feedbackEl.classList.add('error');
                }
                return;
            }

            duplicateLessonFromEdit(plan);
        });
    }

    if (editForm) {
        editForm.addEventListener('submit', handleLessonEditSubmit);
    }
    ...
    modals.forEach(modal => {
        modal.addEventListener('click', event => {
            if (event.target === modal) {
                if (modal.dataset.modal === 'lesson-view') closeLessonViewModal();
                if (modal.dataset.modal === 'lesson-edit') closeLessonEditModal();
            }
        });
    });
}

function duplicateLessonFromView() {
    const modal = document.getElementById('lesson-plan-view-modal');
    if (!modal?.dataset.planId) return;

    duplicateLessonPlan(modal.dataset.planId);
    closeLessonViewModal();
}

function duplicateLessonFromEdit(plan) {
    const duplicated = duplicateLessonPlanData(plan, { date: plan.date });
    showToast(`Lesson Plan "${duplicated.title}" duplicated!`);
    openLessonEditModal(duplicated, { origin: 'duplicate' });
}

function handleLessonEditSubmit(event) {
    event.preventDefault();
    const form = event.target;
    const feedbackEl = document.getElementById('lesson-edit-feedback');
